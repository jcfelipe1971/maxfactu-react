...
const initialData: Familia[] = [
  { id: '1', descripcion: 'PERFILES', tipoIva: 'Exento', permiteNegativo: false },
  { id: '2', descripcion: 'PERFILES', tipoIva: 'Exento', permiteNegativo: false },
  { id: '3', descripcion: 'PERFILES', tipoIva: 'Exento', permiteNegativo: false },
  { id: '4', descripcion: 'PERFILES', tipoIva: 'Exento', permiteNegativo: false },
  { id: '5', descripcion: 'PERFILES', tipoIva: 'Exento', permiteNegativo: false },
  { id: '6', descripcion: 'PERFILES', tipoIva: 'Exento', permiteNegativo: false },
];
...
function Familias() {
  ...
  const handleTipoIvaChange = (id: string, nuevoValor: string) => {
    setFamilias((prev) =>
      prev.map((fam) => (fam.id === id ? { ...fam, tipoIva: nuevoValor } : fam))
    );
  };

  const handlePermiteNegativoChange = (id: string, valor: boolean) => {
    setFamilias((prev) =>
      prev.map((fam) => (fam.id === id ? { ...fam, permiteNegativo: valor } : fam))
    );
  };
                  ...
                  <tr key={familia.id}>
                    ...
                    <td>
                      <select
                        ...
                        onChange={(e) => handleTipoIvaChange(familia.id, e.target.value)}
                      >
                    ...
                    <td>
                      <div className="checkbox-container">
                        <input
                          ...
                          onChange={(e) => handlePermiteNegativoChange(familia.id, e.target.checked)}
                        />
                        <span className={`checkbox-label ${familia.permiteNegativo ? 'si' : 'no'}`}>
